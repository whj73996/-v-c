...
var eventBus = (0, _jquery.default)({}); //————eventBus  将空对象变成JQuery对象，初始化时，在该对象身上on绑定自定义事件（并在回调函数里调用渲染方法），然后在m模块数据增删改查方法中，调用eventBus.trigger('自定义方法')
...
var view = {
  ...
  init: function init(container) {
    ...
    view.autoBindEvents();
    eventBus.on('data-update', function () {
      view.render(m.data.n);
    });
  },
  event: {
    ...
    "click #minus1": "min",
    "click #mul2": "mul",
    "click #divide2": "div" //表驱动编程

  },
  render: function render(n) {
    if (view.el.children.length !== 0) view.el.empty(); //container不为空就清空再插入DOM对象

    view.el = (0, _jquery.default)(view.html.replace("{{number}}", n)).appendTo(view.el); //将新的DOM对象放在v.el中，并插入（因为外面有层container,不怕插错地方）
    //以新换旧
  },
  add: function add() {
  ...
  autoBindEvents: function autoBindEvents() {
    var _loop = function _loop(key) {
      //表驱动编程，就是把事件做成hash表，然后遍历hash表绑定方法，这样可以稳定工作量
      var spaceIndex = key.indexOf(" ");
      var event = key.slice(0, spaceIndex);
      var ele = key.slice(spaceIndex + 1);
      view.el.on(event, ele, function (e) {
        view[view.event[key]](e); // v.render(m.data.n)
      });
    };

    for (var key in view.event) {
      _loop(key);
    }
  }
}; //第一次渲染

var _default = view; //暴露接口给外部，模块化调用的核心步骤
...
var view = {
  ...
  init: function init(container) {
    view.el = (0, _jquery.default)(container); //c.init(）是启动方法
  ...
  autoBindEvents: function autoBindEvents() {
    var _loop = function _loop(key) {
      ...
      var ele = key.slice(spaceIndex + 1);
      view.el.on(event, ele, function (e) {
        view[view.event[key]](e); // c.render(m.data.n)
      });
    };
